    ...
    try {
        ...
        for ($i = 1; $i -le $SlotCount; $i++) {
            try {
                ...
                $body = $slotData.GetEnumerator() | ForEach-Object { "$($_.Key)=$($_.Value)" } | Out-String -Stream | ForEach-Object { $_.Trim() } | Where-Object { $_ } | Join-String -Separator "&"
                ...
                if ($createResponse.id) {
                    ...
                    $startBody = $startData.GetEnumerator() | ForEach-Object { "$($_.Key)=$($_.Value)" } | Out-String -Stream | ForEach-Object { $_.Trim() } | Where-Object { $_ } | Join-String -Separator "&"
