...
<head>
    <meta charset="UTF-8">
    <title>[{{ $appName }}] New {{ $typeLabel }} Transaction - {{ $poNumber }}</title>
</head>
    ...
    <tr>
        <td align="center">
            <table width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 12px rgba(15,23,42,0.12);">
                <tr>
                    <td style="padding:20px 24px;background:linear-gradient(135deg,{{ $slotType === 'unplanned' ? '#92400e,#d97706' : '#065f46,#059669' }});color:#ffffff;">
                        <table width="100%" cellpadding="0" cellspacing="0">
                            ...
                            <tr>
                                <td style="padding-top:12px;">
                                    <div style="font-size:18px;font-weight:600;">New {{ $typeLabel }} Transaction</div>
                            ...
                            <tr>
                                <td style="padding-top:8px;">
                                    <div style="font-size:14px;">Dear {{ $notifiable->name ?? 'Admin' }},</div>
                                    <div style="font-size:12px;opacity:0.8;margin-top:4px;">{{ $notifiable->roles->first()->name ?? 'Approver' }} – PT Oneject Indonesia</div>
                                </td>
                            </tr>
                        </table>
                    </td>
                ...
                <tr>
                    <td style="padding:24px;">
                        <p style="margin:0 0 16px 0;color:#374151;">A new <strong>{{ strtolower($typeLabel) }}</strong> transaction has been created by <strong>{{ $createdByName }}</strong> (internal team).</p>
                            ...
                            <tr>
                                <td style="padding:12px;background-color:#f8fafc;border-radius:8px;">
                                    <table width="100%" cellpadding="0" cellspacing="0">
                                        ...
                                        @if($truckType)
                                        <tr>
                                            <td style="padding:4px 0;font-size:14px;"><strong>Truck Type:</strong> {{ $truckType }}</td>
                                        </tr>
                                        @endif
                                        @if($vehicleNumber)
                                        <tr>
                                            <td style="padding:4px 0;font-size:14px;"><strong>Vehicle:</strong> {{ $vehicleNumber }}</td>
                                        </tr>
                                        @endif
                                        <tr>
                                            <td style="padding:4px 0;font-size:14px;"><strong>Created By:</strong> {{ $createdByName }}</td>
                                        </tr>
                                    </table>
                                </td>
                            </tr>
                        ...
                        @php
                            $detailUrl = $slotType === 'unplanned'
                                ? route('unplanned.show', $slotId)
                                : route('slots.show', ['slotId' => $slotId]);
                        @endphp
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0">
                            <tr>
                                <td align="center" style="padding:16px 0;">
                                    <a href="{{ $detailUrl }}"
                                       style="display:inline-block;background-color:{{ $slotType === 'unplanned' ? '#d97706' : '#059669' }};color:#ffffff;padding:12px 24px;text-decoration:none;border-radius:6px;font-weight:500;">
                                        View Transaction
                                    </a>
                                </td>
                            </tr>
                        ...
                        <p style="margin:16px 0 0 0;color:#6b7280;font-size:13px;">This is an informational notification. No approval action is required.</p>
                    </td>
                </tr>
