<div class="st-card st-mb-16 st-border-l-4 st-card--primary-accent">
    ...
    <div class="st-form-row--grid-3 st-text--sm">
        @php
            $docType = 'PO / SO Number';
            $isOutbound = strtolower($slot->direction ?? '') === 'outbound';
            if ($isOutbound) {
                $docType = 'SO Number';
            } elseif (strtolower($slot->direction ?? '') === 'inbound') {
                $docType = 'PO Number';
            }
        @endphp
        <div class="st-flex st-align-center st-gap-8">
            ...
            <div>
                ...
                <div class="st-flex st-flex-col st-gap-2 st-mt-2">
                    @if(isset($slot->planned_start))
                        ...
                        <div class="st-font-semibold st-flex st-align-center st-gap-6"><i
                                class="far fa-calendar-alt st-text--slate st-text-12"></i> {{ $eta->format('d-m-Y') }}</div>
                        <div class="st-font-semibold st-flex st-align-center st-gap-6"><i
                                class="far fa-clock st-text--slate st-text-12"></i> {{ $eta->format('H:i') }}</div>
                    @else
    ...
    <form method="POST"
        ...
        <div class="st-form-row st-form-field--mb-12">
            <div class="st-form-field">
                ...
                <div class="st-flex st-gap-8 st-align-center">
                    <input type="text" name="ticket_number" class="st-input" required value="{{ old('ticket_number') }}"
                        placeholder="Scan barcode or enter ticket number...">
                    <button type="button" id="btn_scan_ticket" class="st-btn st-btn--secondary st-btn--pad-md st-nowrap"
                ...
                <div id="scan_camera_wrap" class="st-hidden-soft st-mt-8 st-border st-rounded-8 st-p-8 st-bg-slate-50">
                    <div class="st-flex st-align-center st-gap-8 st-justify-between">
                        ...
                        <button type="button" id="btn_scan_stop"
                            class="st-btn st-btn--secondary st-btn--xs st-btn--pad-sm" title="Stop Camera">
                            <i class="fas fa-stop"></i>
                        </button>
                    </div>
                    <video id="scan_camera" class="st-w-full st-maxw-360 st-mt-8 st-rounded-6 st-scale-x-neg" autoplay
                        muted playsinline></video>
                    <div id="scan_qr_reader"
                        class="st-w-full st-maxw-360 st-mt-8 st-rounded-6 st-hidden-soft st-scale-x-neg"></div>
                ...
                <div class="st-flex st-justify-between st-align-center st-mt-4 st-gap-10">
                    <div class="st-text--small st-text--muted st-flex-1">After ticket is filled, detail form will
                        appear.</div>
                    <a href="{{ url()->previous() !== url()->current() ? url()->previous() : route('slots.index') }}"
                        class="st-btn st-btn--outline-primary st-btn--xs st-flex-0 st-nowrap">Back</a>
                </div>
            </div>
        </div>
        ...
        <div id="arrival_details" class="st-hidden-soft">
                ...
                <div class="st-form-row--grid-2 st-gap-24">
                    ...
                    <div class="st-flex st-flex-col st-gap-16">
                        ...
                        <div>
                            ...
                            <div class="st-font-semibold st-text-15">
                                {{ app(\App\Services\SlotService::class)->getGateDisplayName($slot->planned_gate_warehouse_code ?? '', $slot->planned_gate_number ?? '') }}
                            </div>
                        </div>
                        <div>
                            ...
                            <div class="st-flex st-flex-col st-gap-4">
                                @if(isset($slot->planned_start))
                                    ...
                                    <div class="st-font-semibold st-text-15 st-flex st-align-center st-gap-8"><i
                                            class="far fa-calendar-alt st-text--muted st-text-14"></i>
                                        {{ $etaDetail->format('d-m-Y') }}</div>
                                    <div class="st-font-semibold st-text-15 st-flex st-align-center st-gap-8"><i
                                            class="far fa-clock st-text--muted st-text-14"></i>
                                        {{ $etaDetail->format('H:i') }}</div>
                                @else
            ...
            <div class="st-form-actions">
                ...
                <button type="button" class="st-btn st-btn--outline-primary st-btn--pad-lg" title="Cancel"
                    onclick="closeGlobalAjaxModal()">
